...
process ProcessCreate(const char* program, pipeT* stdin_pipe, pipeT* stdout_pipe) {
    ...
    if (pid == 0) {
        ...
        // РОДИТЕЛЬСКИЙ ПРОЦЕСС - ЗАКРЫВАЕМ НЕНУЖНЫЕ КОНЦЫ ПАЙПОВ
        if (stdin_pipe) {
            close(stdin_pipe->read_end);  // Закрываем, т.к. дочерний процесс использует его
        }
        if (stdout_pipe) {
            close(stdout_pipe->write_end); // Закрываем, т.к. дочерний процесс использует его
        }
