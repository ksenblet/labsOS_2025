...
int main() {
    ...
    // создаем процессы(кроссплатформенность выполняется)
    #ifdef _WIN32
...
#   else
    ...
    // закрываем write-end пайпов, чтобы сигнал детям поступил о завершении
    PipeCloseWriteEnd(&pipe1);
    PipeCloseWriteEnd(&pipe2);
    PipeCloseWriteEnd(&pipe3);
    
    // завершение детей, код выхода 1 = не корректно, но завершились(зомби)
    if (child1.is_valid) {
